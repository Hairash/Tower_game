    ...
    def run(self, screen):
        ...
        while self.running:
            ...
            zoomed_field = pygame.transform.smoothscale(
                self.field.surface,
                (
                    int(self.field.width * self.camera.zoom_scale),
                    int(self.field.height * self.camera.zoom_scale),
                )
            )
            # print(
            #     self.camera.offset.x,
            #     self.camera.offset.y,
            #     self.camera.screen_width / self.camera.zoom_scale,
            #     self.camera.screen_height / self.camera.zoom_scale,
            # )
            ...
            # )
            # screen.blit(zoomed_field, (0, 0))
            screen.blit(zoomed_field, -self.camera.offset)
